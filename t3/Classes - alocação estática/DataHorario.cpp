...
namespace poo{
    ...
    DataHorario::DataHorario(int _dia, int _mes, int _ano, int _hora, int _minuto, int _segundo){
        ...
        if (dataHorario_Valido){
            ...
            // Se todos os parâmetros foram passados corretamente, a data é configurada com eles
            if (dataHorario_Valido){
                this->dia = _dia;
                this->mes = _mes;
                this->ano = _ano;
                ...
                // Configurando os atributos para o padrão 01/01/0001 00:00:00
    ...
    bool DataHorario::operator<(const DataHorario & dh_direita) const {
        // Obtendo os atributos para realizar as comparações
    ...
    bool DataHorario::operator==(const DataHorario & dh_direita) const{
        // Obtendo as expressões para análise da igualdade
        bool expressoes[6];
        ...
        // Verificando a igualdade com o vetor de auxílio
        int indice = 0;
        while (indice < 6){
